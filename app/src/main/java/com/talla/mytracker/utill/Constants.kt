package com.talla.mytracker.utill

object Constants
{
    const val LOCATION_REQUEST_PERMISSION_CODE=1
    const val BACKGROUND_LOCATION_REQUEST_PERMISSION_CODE=2
    const val ACTION_SERVICE_START="ACTION_SERVICE_START"
    const val ACTION_SERVICE_STOP="ACTION_SERVICE_STOP"
    const val ACTION_NAVIGATE_TO_MAPS_FRAGMENT="ACTION_NAVIGATE_TO_MAPS_FRAGMENT"

    const val NOTIFICATION_CHANNEL_ID="TRACKER_NOTIFICATION_CHANNEL_ID"
    const val NOTIFICATION_CHANNEL_NAME="NOTIFICATION_CHANNEL_NAME"
    const val NOTIFICATION_ID=3
    const val PENDING_INTENT_REQUEST_CODE=4

    const val LOCATION_UPDATE_INTERVAL=4000L
    const val LOCATION_FASTEST_UPDATE_INTERVAL=2000L

}